...
export default function SMTPPage() {
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-extrabold text-white tracking-tight">Configurações SMTP</h1>
        <p className="text-slate-400 mt-1">Configure o serviço de e-mail para envio de alertas do sistema e notificações críticas.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          {/* Main Config Form */}
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl lg:col-span-2 shadow-xl">
            <form onSubmit={handleSave} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-2">
                  <label className="block text-slate-300 text-xs font-semibold uppercase tracking-wider mb-2">Servidor Host SMTP</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white placeholder-slate-700 focus:outline-none focus:border-blue-500 text-sm transition-colors"
                    />
                  </div>
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-slate-300 text-xs font-semibold uppercase tracking-wider mb-2">Usuário / E-mail Autenticação</label>
                  <input
                    type="text"
                    required
                    value={user}
              ...
              <div>
                <label className="block text-slate-300 text-xs font-semibold uppercase tracking-wider mb-2">E-mail de Remetente (FROM)</label>
                <input
                  type="email"
                  required
                  value={from}
              ...
              <div className="flex items-center gap-2 pt-2">
                <input
                  ...
                  className="w-4 h-4 bg-slate-950 border-slate-850 rounded text-blue-600 focus:ring-blue-500 cursor-pointer"
                />
                <label htmlFor="secure" className="text-sm text-slate-300 cursor-pointer select-none">
                  Requer conexão segura SSL/TLS (Usar na porta 465)
                </label>
              </div>

              <div className="border-t border-slate-800 pt-5 flex justify-between gap-3">
                <button
                  type="button"
                  onClick={() => setShowTestPrompt(true)}
                  className="px-4 py-2.5 bg-slate-950 hover:bg-slate-800 border border-slate-850 hover:border-slate-700 text-slate-300 hover:text-white rounded-xl text-sm font-semibold transition-colors cursor-pointer flex items-center gap-1.5"
                >
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-xl">
            ...
            <div className="space-y-3 pt-2 text-xs text-slate-300">
              <div className="p-3 bg-slate-950 border border-slate-850 rounded-xl">
                <span className="font-bold text-white block mb-1">Ações Críticas</span>
                Disparos automáticos ocorrem no desligamento (STOP) ou reinicialização (REBOOT) de qualquer máquina virtual.
              </div>
              <div className="p-3 bg-slate-950 border border-slate-850 rounded-xl">
                <span className="font-bold text-white block mb-1">Alterações no Nó</span>
      ...
      {showTestPrompt && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md shadow-2xl relative animate-scale-up">
            ...
            <form onSubmit={handleTestEmail}>
              <div className="p-6">
                <label className="block text-slate-300 text-xs font-semibold uppercase tracking-wider mb-2">E-mail Destinatário</label>
                <input
                  type="email"
                  required
                  value={testEmail}
              ...
              <div className="p-6 border-t border-slate-800 flex justify-end gap-3 bg-slate-900/50 rounded-b-2xl">
                <button
                  ...
                  className="px-4 py-2 border border-slate-850 hover:bg-slate-800 text-slate-400 hover:text-white rounded-xl text-xs font-semibold cursor-pointer"
                >
                  Cancelar
                </button>
                <button
