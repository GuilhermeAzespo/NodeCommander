...
export default function VMsPage() {
  ...
  const [selectedVmForSnapshot, setSelectedVmForSnapshot] = useState<any>(null);
  ...
  return (
    <div className="space-y-8">
                            ...
                            {actionLoadingId === vm.id ? (
                              ...
                              <div className="flex justify-center gap-1.5">
                                ...
                                {canControl && (
                                  <button
                                    onClick={() => {
                                      setSelectedVmForSnapshot(vm);
                                      setSnapshotModalOpen(true);
                                    }}
                                    className="p-1.5 bg-bg-primary hover:bg-blue-500/10 border border-border-color hover:border-blue-500/30 text-text-secondary hover:text-blue-500 rounded-lg transition-colors cursor-pointer"
                                    title="Gerenciar Snapshots"
                                  >
      ...
      <SnapshotManagerModal
        ...
        hypervisorId={selectedHvId}
        vmId={selectedVmForSnapshot?.id || ""}
        vmName={selectedVmForSnapshot?.name || ""}
      />
    </div>
  );
}
