...
export class ProxmoxProvider implements HypervisorProvider {
  ...
  async listSnapshots(vmId: string): Promise<any[]> {
    if (this.isMock) {
      return [
        { name: "initial-setup", description: "Configuração base", snaptime: Math.floor(Date.now() / 1000) - 86400, vmstate: 1 },
        { name: "before-update", description: "Antes do apt upgrade", snaptime: Math.floor(Date.now() / 1000) - 3600, vmstate: 0 }
      ];
    }
    try {
      const node = await this.resolveNodeForVM(vmId);
      const snapshots = await this.request("GET", `/nodes/${node}/qemu/${vmId}/snapshot`);
      // Proxmox returns an array including the 'current' state, we filter it out if we just want saved snapshots
      return Array.isArray(snapshots) ? snapshots.filter((s: any) => s.name && s.name !== "current") : [];
    } catch (err) {
      console.error(`Proxmox listSnapshots failed for ${vmId}:`, err);
      return [];
    }
  }

  async createSnapshot(vmId: string, name: string, description?: string, vmstate?: boolean): Promise<boolean> {
    if (this.isMock) return true;
    try {
      const node = await this.resolveNodeForVM(vmId);
      const params: any = { snapname: name };
      if (description) params.description = description;
      if (vmstate) params.vmstate = 1;
      
      await this.request("POST", `/nodes/${node}/qemu/${vmId}/snapshot`, params);
      return true;
    } catch (err) {
      console.error(`Proxmox createSnapshot failed for ${vmId}:`, err);
      return false;
    }
  }

  async rollbackSnapshot(vmId: string, snapName: string): Promise<boolean> {
    if (this.isMock) return true;
    try {
      const node = await this.resolveNodeForVM(vmId);
      await this.request("POST", `/nodes/${node}/qemu/${vmId}/snapshot/${snapName}/rollback`);
      return true;
    } catch (err) {
      console.error(`Proxmox rollbackSnapshot failed for ${vmId} snap ${snapName}:`, err);
      return false;
    }
  }

  async deleteSnapshot(vmId: string, snapName: string): Promise<boolean> {
    if (this.isMock) return true;
    try {
      const node = await this.resolveNodeForVM(vmId);
      await this.request("DELETE", `/nodes/${node}/qemu/${vmId}/snapshot/${snapName}`);
      return true;
    } catch (err) {
      console.error(`Proxmox deleteSnapshot failed for ${vmId} snap ${snapName}:`, err);
      return false;
    }
  }
